...
核对日期：2026-09-06。以本地 HEAD 和当前工作区源码为准；已有 GPU profiling 单独标为历史实验，不作为本次 NPU 实测或跨路线性能排名。

本文面向项目经理、技术主管、开发者、客户和测试人员，目的在于建立可用于技术分析及后续管理讨论的共同事实基础，而不是选择“获胜团队”。关注顺序为：技术相似性、megaKernel 可达成性、用户感知、表达能力、优化空间、动态能力、开发成本。

**范围约定：**原称 PyPTO（Simpler）的路线，全文改称 **PyPTO3（Simpler）**；这是本文命名，不修改 `npu/pypto` 仓库名。graph-autofusion **只纳入 AutoFuse，排除 SuperKernel**，后者的静态多 kernel 重组能力不计入本文任何能力判断。`pypto-lib` 是 PyPTO3 的算子 / 模型库，不另算第八条路线。
...
| --- | --- | --- | --- |
| A（PyPTO2-普通版） | `npu/pypto2/python/pypto` + `framework` | Tensor 程序编译、自有 IR、TileFwk 任务体系 | 设备侧控制 / 依赖解析 / 任务派发 |
| B（PyPTO2-Pro） | 同仓 `python/pypto_pro` | 显式 Tile / 核内编程，自有 IR → CCE | Host 直接 launch 多核 kernel；kernel 内划分工作 |
| C（PyPTO3（Simpler）） | `npu/pypto` + `runtime`；用户例子优先 `npu/pypto-lib` | 多层程序 / InCore / Orchestration + PTOAS + Simpler | 任务图；也能把混合核 SPMD kernel 作为一个多 block task |
| D（CANNBot DSL） | `npu/cannbot-dsl_ma` | Python 分阶段 Host / Device 编程，MLIR CANNIR → AscendC | Host launch；用户显式内存、Channel 和核内流水 |
| E（PyPTO on GPU） | `gpu/PyPTO-LOVE-TensorIR` 的集成层及 `.sources/pypto` | PyPTO 前端 / 自有 IR → 受支持模式 → NVIDIA TensorIR / CUDA Tile IR | PyPTO GPU runtime 直接 CUDA launch；硬件安排 CTA |
| F（Triton-Ascend） | `npu/triton-ascend`；Inductor 入口 `npu/torch_npu/torch_npu/_inductor` | Triton kernel DSL，也可接受 Inductor 生成的 kernel | 编译器映射逻辑 program 到 NPU；CANN launch |
| G（AutoFuse + Inductor） | `npu/graph-autofusion/autofuse` + `npu/torchair/experimental/_inductor_npu_ext` | 融合图编译 / 自动 tiling / kernel 生成组件；客户通过 PyTorch 使用 | Inductor 分组及 Host wrapper；生成的多核融合 kernel |
...
3. **F（Triton-Ascend） 与 G（AutoFuse + Inductor） 最接近的是“PyTorch → Inductor → 自生成 NPU kernel”的集成位置。** F（Triton-Ascend） 还拥有明确的独立 kernel DSL；G（AutoFuse + Inductor） 没有同等定位的终端用户 kernel DSL，但不是“没有用户入口”。
4. **E（PyPTO on GPU） 的前端血缘接近 C（PyPTO3（Simpler）），当前执行与核内编译分工更接近 F（Triton-Ascend）。** E（PyPTO on GPU） 没有把 Simpler scheduler 移植为 GPU runtime；多 SM 主要来自一个 launch 内的多个 CTA。
5. **PTOAS、AscendNPU-IR、NVIDIA TensorIR、AscendC 不能与上述七条上层路径当作平级替代品。** 前三者是不同层次的编译基础设施；AscendC 主要是设备编程 API / 库及配套编译接口，不是任务调度框架。
6. **MLIR 和 SSA 不是对立选项；MLIR 的表示能力、已实现 pass 和运行时能力也不是同一件事。** A（PyPTO2-普通版）/B（PyPTO2-Pro）/C（PyPTO3（Simpler））/E（PyPTO on GPU） 的前端可以使用自有 SSA IR；C（PyPTO3（Simpler）） 在后端交 PTO dialect MLIR，E（PyPTO on GPU） 交 TensorIR；D（CANNBot DSL）/F（Triton-Ascend） 在更长的编译区间使用 MLIR；G（AutoFuse + Inductor） 的 ASCIR 图不是因为名字有 IR 就成为 MLIR。
7. **Pro 以 SPMD kernel 为主，PyPTO3 的外层则支持 MPMD 任务编排；两者不是互斥标签。** C（PyPTO3（Simpler）） 的 `pypto-lib` 已有多 block、混合 Cube/Vector、带显式任务依赖的 SPMD attention；框架与其中一个 kernel 的执行方式属于不同层。第 4.10 节展开用户分工、设备派发及组合边界。
8. **动态 shape 不必要求客户另写 tiling 函数。** B（PyPTO2-Pro）/D（CANNBot DSL）/F（Triton-Ascend） 可以在固定物理 tile 上循环并处理有效长度；G（AutoFuse + Inductor） 会生成 Host tiling；A（PyPTO2-普通版）/C（PyPTO3（Simpler）） 也仍然存在 tiling 决策，甚至有显式 tiling task。
...
10. **当前没有足以给七条路线做性能名次的同口径实验。** 本文可以判断已有实现、可行的组合边界和可能代价，不能据源码长度、MLIR 使用量、launch 数或核心利用率推导谁更快。
...
本章保留数学参考、原生 kernel、Host 调用及源码入口，按下列四类标注完整性。**全文的原生 NPU 代码均未经本机编译执行；已做的 CPU 检查、历史 GPU 实验及未验证事项统一列于第 12 章。**“完整代码”不等于“已实测”，注明“摘录”的代码仍依赖所列上下文。版权与许可证以原文件为准。
...
| 决策 | Tensor / task 路线 | 显式核内路线 | Triton 路线 | AutoFuse 客户路线 |
| --- | --- | --- | --- | --- |
| 遍历多少 page | A（PyPTO2-普通版） 的程序控制流，C（PyPTO3（Simpler）） 的 Orchestration 或 SPMD 内循环 | B（PyPTO2-Pro）/D（CANNBot DSL） kernel 从 GM 读取长度后循环 | `tl.load(seq_lens)` → runtime loop | padded mask 或受支持图算子；能否降为紧凑动态循环取决于 lowering |
| QK / PV | Tensor matmul 或独立 InCore | 显式 Cube / memory stage / pipeline | `tl.dot`，后端安排 Cube / Vector 交互 | `torch.matmul` / 模板 / 外部库，须查生成归属 |
| m/l/o 放哪里 | 编译器规划的 tensor 或 task 间 GM | UB / registers / GM transfer，由用户与 passes 分工 | 编译器 bufferization / workspace / pipeline | AutoFuse schedule、buffer 分配和 workspace |
| 请求分配给核 | 任务派发，或用户选择 SPMD 映射 | core-stride、work_ranges 或自定义协议 | program grid + 编译器映射 | 自动 tiling 决定 blockDim 和 work split |
| 性能相关长度策略 | 可动态生成更多 task；不保证自动负载均衡最优 | 可传 Host 计算的 work_ranges，也可设备侧计算 | Host 上界 grid + kernel 读取真实长度 | Host generated tiling 通常处理元数据；tensor 内容不能自动当成 Host shape |
...
#### B（PyPTO2-Pro） 的完整 paged attention 例子是 prefill，不能偷换成 decode 实测
...
#### D（CANNBot DSL）：有 paged addressing 的设计和基础构件，未找到独立完整 PA 示例

[D12（CANNBot DSL）] 文档开头明确写明当前 examples 不包含独立 PagedAttention 实现；后面仍有阶段性实现记录，不能越过开头和当前文件树，直接宣称已有可运行的 decode 示例。
...
移植为 paged decode 需将连续 KV 寻址改为设备查表、处理实际长度尾块，并重新评估 q_len=1 的 GQA 小 M 打包及 page_size/tile_n 关系。
...
当前报告没有“完整目标指令清单 × 所有参数组合”的分母，也没有覆盖该集合的端到端测试。因此不提供 API/ISA 覆盖百分比，不写“全部支持”。这不表示七条路线同样受限：**Pro/CANNBot 更直接暴露低层选项，普通版/AutoFuse 更多交给编译器，PyPTO3 同时提供 Tensor/Tile 核内计算与带依赖的任务编排接口，GPU 集成还明显受 emitter 覆盖约束**；这是控制面的差异，不是未经测量的完整性名次。
...
从客户感知看，封装后的七条路线都可能只需一行函数调用。真正区分它们的是：**新增不支持的组合、新 shape 策略或硬件特性时，客户/算子作者/编译器团队分别需要改哪一层**。第 4.8—4.10 节把这一点落实到两组重点路线，第 10 章再讨论是否能替换或共用对应层。
...
| 层 | A（PyPTO2-普通版） | B（PyPTO2-Pro） | C（PyPTO3（Simpler）） | D（CANNBot DSL） | E（PyPTO on GPU） | F（Triton-Ascend） | G（AutoFuse + Inductor） |
| --- | --- | --- | --- | --- | --- | --- | --- |
| L0 客户入口 | Tensor DSL | Tile / VF / Cube DSL | Tensor / Tile、多级 scope | Python Host / kernel / Channel | PyPTO DSL + operator API | Triton DSL；或 PyTorch | 主要是 PyTorch；开发者 ASCIR 接口 |
| L1 框架入口 | 自有 JIT/运行接口；不据此认定 Inductor 原生后端 | 直接 kernel 接口 | 自有 JIT / model zoo | JIT/AOT Host 接口 | 集成包 `compile_graph` | 指定 torch_npu Inductor 路径 | 指定 TorchAir experimental Inductor 路径 |
| L2 分组与程序 | TileFwk 图 / 函数 / task 编译 | 用户 kernel 边界及核内 passes | InCore / Orchestration / Group / SPMD | Host 与 Device 分阶段 | 当前支持的单图模式；包装层可多 launch | Inductor 融合或用户独立 kernel | Inductor 分组 + AutoFuse schedule |
| L3 主 IR | PIL / 自有 IR → TileFwk | 共享基础自有 IR + Pro 操作 | 自有 IR → PTO MLIR | MLIR CANNIR / Asc / vector 等 | 自有 IR → NVIDIA TensorIR | TTIR → 适配 MLIR → HFusion/HIVM 等 | ASCIR / FusedScheduledResult |
| L4 tiling / 映射 | tile 设置 + 编译分解 | 物理 tile、block-stride、Host 策略 | 编译 tile + runtime task；可手写 SPMD | tile 与 Host/block 分工显式 | schedule tile / pattern → grid | constexpr tile + grid + 后端再映射 | 自动模板 / cost model / ATT tiling |
| L5 片上规划 | 图编译及核内 codegen | 显式地址/TileGroup + passes | PyPTO memory/layout passes + PTOAS | Buffer / Channel + MLIR passes | TensorIR / Tile 编译器 | AscendNPU-IR PlanMemory/layout | buffer/queue allocator、reuse、代码生成 |
| L6 流水与同步 | 编译生成及用户策略 | section、mutex、pipeline、显式同步 | InCore passes / PTOAS；可显式混合核事件 | Channel、软件流水、同步推断 | GPU lowering / tile compiler | CV pipeline、MultiBuffer、InjectSync/GSS | schedule、queue、生成的同步与模板 |
...
| 层 | A（PyPTO2-普通版） | B（PyPTO2-Pro） | C（PyPTO3（Simpler）） | D（CANNBot DSL） | E（PyPTO on GPU） | F（Triton-Ascend） | G（AutoFuse + Inductor） |
| --- | --- | --- | --- | --- | --- | --- | --- |
| L7 常见末端 | CCE codegen + 工具链 | CCECodegen + 工具链 | PTOAS 默认 C++ 路径 + PTO tile 库 / 工具链 | AscendC source + 工具链 | TensorIR → CUDA Tile IR → Cubin | bishengir/HIVM 及目标编译器 → npubin | 生成 AscendC / 相关模板 → 工具链 |
| L8 launch | CTRL/SCHE/AICore 组合，视配置 | `kernel<<<blockDim,...,stream>>>` | runtime worker + `kernel_entry(args)` task ABI | Host C++ 的 `kernel<<<blockDim,0,stream>>>` | `cuLaunchKernelEx`，显式 grid / artifact block / stream | `cann_launch_kernel(func,blockNum,stream,...)` | wrapper 调 tiling、分配 workspace、调用 launch |
| L9 跨任务主链 | TileFwk 设备任务系统 | 没有默认通用 ready-queue scheduler | Simpler，含多 block task 与依赖 | 没有默认 Simpler 式 scheduler | CUDA kernel/stream 与硬件 CTA 调度 | Host / stream；非 AICPU 任务图主链 | Inductor Host/stream；非 AICPU 任务图主链 |
| L10 kernel 外 GM | 图/runtime 内存计划 + 用户 tensor | 调用者/框架分配 + 传参 | tensor planner / scope/runtime + 用户显式 GM | Host/调用者；AOT contract | torch 集成分配，shape 固化 | 调用者或 Inductor；wrapper 分配内核 scratch | Inductor buffer + wrapper workspace |
| 跨核协调 | runtime依赖 + kernel内部协议 | mixed kernel / 显式同步 | runtime依赖 + SPMD内部事件/屏障 | Channel / mixed-core 同步等 | CTA内同步；跨CTA必须另有合法机制 | 编译器生成CV/跨block同步等 | 生成的核内/跨核协议与workspace |
...
Paged attention 中，两者都可把真实长度作为数据、以固定物理 tile 循环处理；Host 均衡分工是否随长度更新，取决于算法设计。Pro 已找到的 paged prefill 代码有 `build_work_ranges`；CANNBot 已找到的是 FlashAttention/Cube 通路，尚无本次所需独立完整原生 paged decode 的同级证据。因此这里只能比较机制与实现差距，不能按“attention”名称直接声称等价覆盖或等价性能。
...
这里的 block ID 是逻辑索引，不应承诺它永久绑定特定物理核；mixed kernel 的 Vector 域还需按 subblock 语义计算工作单元数。`num_cores=32` 等样例常量也不是所有芯片通用的硬件结论。这个 softmax 测试标记的目标为 `soc("950")`，本文未在本机执行。[B13（PyPTO2-Pro）] [B14（PyPTO2-Pro）]
...
2. **paged decode：外层调度与核内动态长度要分别设计。**按 Pro 的 kernel 编程方式，作者需要在合作 kernel 中设计 QK、softmax、PV 及实际长度循环，或通过 Host 拆成多个 launch；第 2.6 节的完整 paged 示例是 prefill，不将其升级为 decode 已实测。PyPTO3 既可将阶段拆成不同任务，也可像 `pypto-lib` native PA 一样保留为融合的多 block SPMD task。后者一旦将请求按固定 stride 分给 logical blocks，外层 Simpler 就不会自动把长请求的剩余 pages 迁移给短请求已经空闲的核。拆成更细任务可能增加调度机会，也可能增加中间 GM、依赖和调度开销；应按算法/数据生命周期决定，而不是一律拆开。
...
**MLIR 的表达上界并不止于算子图，下界也不止于循环；但“能够建模”不等于“现有工具链能够优化并执行”。** 比较七条路线时，应分别回答三个问题：
...
| 更低层模型 | 框架允许继续定义更低层抽象，并无“必须止于 LLVM”的概念边界 | 不是说本地七条路线都实现了这些层，更不是 MLIR 自己执行硬件 |
...
以两个极小片段说明（本文构造的语义示意，不是七条路线某次编译的 dump，也未做工具链编译）：
...
### 5.3 七条路线：MLIR 从哪里进入，在哪里退出，哪些决策已经做完

下表判断的是**本地所选主调用链**。底层商业编译器内部可能使用其他 IR，不将未审计的内部过程推测为七条前端“直接使用 MLIR”。
...
同时，PTOAS 也应限定语境：**PyPTO3 当前将 PTOAS 用作核内实现编译器，不把 Simpler 的任务调用顺序交给它；但本地 PTOAS 本体并非只能容纳一个函数，VPTO/fatobj 路径也已有 Host stub 生成。**“不生成上层调用程序”与“完全没有 Host 相关产物”不是同一句话。下面依据本地源码区分这些边界，未在本机执行 NPU 编译或 launch。
...
| D（CANNBot DSL） | Host/Device DSL、完整FA混合流水、显式缓冲与控制 | 手写大合作kernel有表达基础；也可由Host组合多kernel | 缺少本次范围内完整PA decode/整层原生例子；不能仅凭API覆盖就宣布完成 |
...
数值精度、KV cache 布局/复用、并发负载和硬件必须一致才可比较。本文没有七条路线同口径实测，下面给出的是**源码约束下的性能机会与成本模型**，不是预测的加速百分比或成熟度评分。
...
| 大合作 kernel / persistent SPMD，含角色分工 | 阶段间直接通信、细粒度流水和局部复用；减少部分调度边界 | 全局同步、资源最重分支、代码/寄存器压力、动态不均衡、进展/退出协议 | B（PyPTO2-Pro）、D（CANNBot DSL）；C（PyPTO3（Simpler））的 SPMD task；F（Triton-Ascend）须在后端/协议支持范围内 |
...
- **具体差距**：当前 PA 仍有三块 GM transfer；`manual_scope` 下跨 task 依赖必须手工正确；批次大于 `BATCH_PAD` 时按窗口连续推进，scratch-ready 依赖约束复用，不能据“支持更大 batch”推出已经最佳吞吐；本次 PA 驱动主要 A2/A3，A5 不能照搬实测结论。应测整层关键链、窗口串行化、跨阶段流量、scheduler 成本及并发安全，再评估能否缩减 GM/同步或改变分工。

#### D（CANNBot DSL）：适合表达深度流水，当前证据尚不能覆盖原生 paged decode 和整层
...
- **差距**：未找到本次要求的独立完整 native PA decode，不能跳过这一段直接评为整层已就绪。应先补查页/长度/GQA/长尾 decode，再验证两种不同分工阶段间的合作及性能；`@aicpu` 能编 AICPU 函数不等于已有任务 runtime。
...
| --- | --- | --- |
| 完整数学/状态语义 | 整层数值、KV cache 写入、残差、尾块与 shape 契约 | C（PyPTO3（Simpler））有模型程序；A（PyPTO2-普通版）有组合程序；其他路线的局部例子不能替代完整层验收 |
...
| 提交与调度收敛 | Host API、设备 entry、控制/worker、内部 task 分别计数 | A（PyPTO2-普通版）/C（PyPTO3（Simpler））已有 device runtime；整层 K2 仍需 trace |
| 跨阶段数据融合 | buffer 生命周期图、分配峰值、实际 GM/L2/HBM 流量 | C（PyPTO3（Simpler））已有显式 transfer，可定位差距；其他后端必须检查生成物，不能只数源码 tensor |
| 全设备合作正确性 | 每个等待的生产者/参与集合、资源驻留/进展、压力测试 | 不能从单 softmax/FA 的正确性升级为整层无死锁保证 |
...
这些是下一步取证方案，不意味着本次已实现这些原型或七条路线已跑通同一测试。
...
**七条路线在各自支持域内，都能将多个 Vector 计算放进同一核内区域，并消除部分中间 GM tensor；区别在谁划区域、选 tile、表达存储与同步。** 第 8.3 节逐项列出用户与编译器的分工，不承诺任意算子链或 shape 都能融合。
...
其他五条路线提供 Python 入口，不等于在本文已验证一条“任意 ATen 程序原样输入，就自动转成该路线融合 kernel”的前端。客户使用已有算子库时可以不接触 kernel DSL；若要自定义融合链，通常需 kernel 作者用该路线的原生语言实现，或另外开发框架 lowering/集成。这项成本应由提供算子或编译服务的一方计算，而非归为“Python 用户自然就会”。
...
原始trace重新解析得到 **5次 `cuLaunchKernelEx`、5个kernel事件**，与summary一致；CUPTI stream ID不是Host raw stream指针，不能拿两者数值直接比较。该实验选择了非默认stream。
...
### 10.3 把 B（PyPTO2-Pro）/D（CANNBot DSL）/F（Triton-Ascend）/G（AutoFuse + Inductor） kernel 接入 Simpler：可行方向，但当前不是直接兼容
...
| 成本承担者 | A（PyPTO2-普通版）/C（PyPTO3（Simpler）） | B（PyPTO2-Pro）/D（CANNBot DSL） | E（PyPTO on GPU）/F（Triton-Ascend） | G（AutoFuse + Inductor） |
| --- | --- | --- | --- | --- |
| 客户写数学程序 | 需学习Tensor/scope/task模型；库封装可降低负担 | 若直接写kernel则门槛高；调用封装库则低 | E（PyPTO on GPU）取决于pattern/API；F（Triton-Ascend）可直接DSL或PyTorch | PyTorch入口最少改写，但可见fallback/动态guard |
| 高性能算子作者 | 同时理解任务粒度和核内分工 | 低层资源/同步/尾块责任更多 | 理解tile program、compiler lowering和特化策略 | 若只用现成图很少写kernel；扩模板/新lowering责任转到后端开发者 |
| 编译器开发者 | 图语义、核内编译、runtime契约联动 | 核内IR、硬件语义与JIT/Host边界 | 跨IR及target lowering、autotune与ABI | Inductor接入+图schedule+tiling+codegen+模板覆盖 |
| runtime开发者 | A（PyPTO2-普通版）/C（PyPTO3（Simpler））的任务依赖/派发/内存/异常体系较重 | 相对集中在JIT/cache/ABI/launch；复杂合作kernel仍有协议 | E（PyPTO on GPU）为CUDA artifact/launch；F（Triton-Ascend）为CANN及compiler metadata | Host wrapper、generated tiling、workspace、框架/stream集成 |
| 测试与维护 | task和kernel两级验证 | 物理资源、同步、shape边界 | 图覆盖/特化/target/toolchain组合 | 前端分组/模板/extern、动态tiling、框架版本组合 |
...
以下汇总源码分析阶段的证据。文稿去重、“多级表达”澄清、第 4.10 节 SPMD/MPMD 执行模型及第 5.7 节后端编译/launch 边界补充仅涉及文档编辑和相关源码核对；结构、语法、引用及内容保留检查已重做，未新增算子数值或硬件性能实验。

| 事项 | 已记录状态 | 能支持什么结论 |
| --- | --- | --- |
| 七条主链、关键分支、IR/launch/tiling源码 | 已检查本地HEAD及相关函数/调用点 | 实现结构、接口约束、责任归属 |
...
| 七路线前端/API 边界 | 已核对公开表面、参数/目标/融合限制、原生计算链及PyTorch入口 | 区分有 Python DSL、构图接口、API/后端支持；不是完整 ISA 覆盖测试 |
...
| 既有GPU原始trace/summary | 此前重新解析旧trace并核对summary；保留历史数据，未重跑profiling | 历史五阶段的5次launch/grid/block/stream |
| NPU原生kernel编译、A2/A3/A5执行 | 未执行 | 不得写成实测支持、数值或性能结论 |
| 七路线整层Transformer、跨硬件性能 | 未执行 | 不提供速度、利用率或成熟度数值排名 |
...
这些测试只验证本文公共golden，不是B（PyPTO2-Pro）/D（CANNBot DSL）/F（Triton-Ascend）等NPU DSL实现的验证。使用的是现有虚拟环境，未安装依赖，未修改各技术仓库代码。
...
当前文档的 58 个 Python 代码块 AST、196 个本地引用的存在性/行号、61 张 Markdown 表格的列数、99 个代码块的围栏及 13 个导航锚点均通过检查。原有 95 个代码块与去重前逐字一致，第 5.7 节新增 1 个源码调用链示意，第 4.10 节新增 2 个 Python 与 1 个 C++ 源码摘录；源码引用、公式及无关章节保留。4 个 MLIR 代码块及 AscendC 摘录未调用目标工具链编译验证；新增的 SPMD/派发片段以及引用的 PTOAS/HACC/HFusion 测试也未在本机编译运行。本次只做 Markdown 结构检查，未使用渲染器或浏览器视觉预览，不将其表述为渲染验收。
...
当前GPU环境足够完成源码和文档工作，不需要为了架构结论切换机器。若要比较A2/A3与A5的实际生成代码、launch、同步行为或延迟，再使用对应NPU环境；一台NPU也不能替另一个硬件桶完成证明。

### 12.3 本文的限制与待确认问题

- HEAD只是源码快照；实际加载的工具链、wheel、CANN库和缓存二进制可能不同，不能以邻接目录推断版本绑定。
- example/test存在是接口和算法证据，不是本次执行结果；某些源码docstring滞后，本文优先函数体。
- 未找到某种例子不等于整个项目永远不能表达它；尤其是D（CANNBot DSL）/G（AutoFuse + Inductor）的paged decode，当前结论限定在本次检索范围。
...
E（PyPTO on GPU）根目录已有未跟踪文件 `benchmarks/operators/profile_softmax_task_pipeline_ada_sm89.py`；本次保留原样。既有trace/summary是本地实验材料，不因位于仓库目录就成为HEAD已跟踪产物。各仓库未做代码改动、submodule更新或工具链安装。
...
[A1（PyPTO2-普通版）]: npu/pypto2/python/pypto/frontend/parser/entry.py#L1188
[A2（PyPTO2-普通版）]: npu/pypto2/python/pypto/frontend/parser/entry.py#L451
[A3（PyPTO2-普通版）]: npu/pypto2/python/pypto/pil/compile_pipeline.py#L29
[A4（PyPTO2-普通版）]: npu/pypto2/framework/src/interface/tensor/ir.cpp#L207
[A4b（PyPTO2-普通版）]: npu/pypto2/framework/src/interface/tensor/ir_finalize.cpp#L39
[A5（PyPTO2-普通版）]: npu/pypto2/framework/src/codegen/codegen.cpp#L21
[A6（PyPTO2-普通版）]: npu/pypto2/framework/src/machine/runtime/launcher/device_launcher.cpp#L482
[A7（PyPTO2-普通版）]: npu/pypto2/python/pypto/frontend/__init__.py#L72
[A8（PyPTO2-普通版）]: npu/pypto2/python/pypto/_controller.py#L46
[A8b（PyPTO2-普通版）]: npu/pypto2/docs/zh/guide/programming_guide/tensor/development/tiling.md#L9
[A9（PyPTO2-普通版）]: npu/pypto2/framework/src/machine/device/dynamic/aicore_manager.h#L994
[A10（PyPTO2-普通版）]: npu/pypto2/python/src/bindings/runtime.cpp#L430
[A11（PyPTO2-普通版）]: npu/pypto2/python/pypto/ir.py#L135
[A12（PyPTO2-普通版）]: npu/pypto2/examples/02_intermediate/operators/softmax/softmax.py#L76
[A13（PyPTO2-普通版）]: npu/pypto2/python/tests/st/test_ctrl_cpu_perf.py#L73
[B1（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L951
[B2（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L601
[B3（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L1595
[B4（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L467
[B5（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/_bootstrap.py#L14
[B5b（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/ir/__init__.py#L36
[B5c（PyPTO2-Pro）]: npu/pypto2/python/pypto/_loader.py#L60
[B6（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/shape_policy.py#L239
[B7（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/tile_based_python_programming/TilingData.md#L22
[B8（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L1395
[B9（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/compilation_and_execution/offline_binary_compilation.md#L139
[B10（PyPTO2-Pro）]: npu/pypto2/framework/src/interface/pypto_pro/backend/backend_cce_ops.cpp#L834
[B11（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/tile_based_python_programming/tail_block_handling.md#L25
[B12（PyPTO2-Pro）]: npu/pypto2/python/tests/ut/pypto_pro/runtime/test_shape_policy_codegen.py#L109
[B13（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/tile_based_python_programming/multi_core_partitioning_and_Tiling.md#L74
[B14（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/tile_vector/test_softmax.py#L77
[B15（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/fa/test_flex_attention_prefill.py#L14
[B16（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/fa/test_fa_tnd_dn.py#L364
[C1（PyPTO3（Simpler））]: npu/pypto/python/pypto/backend/pto_backend.py#L14
[C2（PyPTO3（Simpler））]: npu/pypto/src/codegen/pto/pto_codegen.cpp#L1078
[C3（PyPTO3（Simpler））]: npu/pypto/src/codegen/orchestration/orchestration_codegen.cpp#L198
[C4（PyPTO3（Simpler））]: npu/pypto/python/pypto/runtime/task_interface.py#L17
[C4b（PyPTO3（Simpler））]: npu/pypto/python/pypto/runtime/worker.py#L220
[C5（PyPTO3（Simpler））]: npu/pypto/runtime/src/a2a3/runtime
[C6（PyPTO3（Simpler））]: npu/pypto/python/pypto/language/scope.py#L210
[C7（PyPTO3（Simpler））]: npu/pypto/python/pypto/language/typing/dynamic.py#L20
[C8（PyPTO3（Simpler））]: npu/pypto/python/pypto/jit/cache.py#L225
[C9（PyPTO3（Simpler））]: npu/pypto/examples/models/06_paged_attention_dynamic.py#L262
[C10（PyPTO3（Simpler））]: npu/pypto/src/ir/transforms/init_memref.cpp#L576
[C11（PyPTO3（Simpler））]: npu/pypto/runtime/tests/st/a2a3/tensormap_and_ringbuffer/spmd_paged_attention_highperf/kernels/pa_tiling.py#L255
[C12（PyPTO3（Simpler））]: npu/pypto/runtime/tests/st/a2a3/tensormap_and_ringbuffer/spmd_paged_attention_highperf/kernels/orchestration/paged_attention_highperf_orch.cpp#L63
[C13（PyPTO3（Simpler））]: npu/pypto/docs/en/dev/language/04-external-kernels.md#L15
[C14（PyPTO3（Simpler））]: npu/pypto/runtime/src/a2a3/runtime/tensormap_and_ringbuffer/common/intrinsic.h#L53
[C15（PyPTO3（Simpler））]: npu/pypto-lib/examples/intermediate/softmax.py#L24
[C16（PyPTO3（Simpler））]: npu/pypto-lib/models/qwen3_14b/paged_attention_pypto.py#L67
[C17（PyPTO3（Simpler））]: npu/pypto-lib/models/qwen3_14b/test_paged_attention_pypto.py#L161
[C18（PyPTO3（Simpler））]: npu/pypto-lib/docs/models/qwen3_14b/paged_attention_pypto.md#L1
[C19（PyPTO3（Simpler））]: npu/pypto-lib/models/qwen3_14b/paged_attention_cce.py#L172
[C20（PyPTO3（Simpler））]: npu/pypto-lib/models/qwen3_14b/decode_fwd.py#L287
[D1（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/lang/jit.py#L11
[D2（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/core/compiler/kernel_materialize.py#L155
[D3（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/core/compiler/builder.py#L419
[D3b（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/core/compiler/passes.py#L23
[D4（CANNBot DSL）]: npu/cannbot-dsl_ma/lib/CANNIR/Translate/TranslateToAscendC.cpp#L2854
[D5（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/aicpu/kernel.py#L9
[D6（CANNBot DSL）]: npu/cannbot-dsl_ma/docs/design/dynamic_shape_design.md#L89
[D7（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannbotdsl/test_aot_dynamic_shape.py#L35
[D8（CANNBot DSL）]: npu/cannbot-dsl_ma/docs/design/datastruct.md#L3
[D9（CANNBot DSL）]: npu/cannbot-dsl_ma/docs/design/jit_aot_end_to_end.md#L102
[D10（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannbotdsl/test_softmax_buffer.py#L25
[D11（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannbotdsl/test_softmax_npu.py#L39
[D12（CANNBot DSL）]: npu/cannbot-dsl_ma/docs/paged_attention_design.md#L1
[D13（CANNBot DSL）]: npu/cannbot-dsl_ma/examples/flash_atten/test_flash_attention_channel_first_swp_npu.py#L1
[E1（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/src/pypto_kernels/_boot.py#L365
[E2（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/src/codegen/nvidia/tensor_ir_codegen.cpp#L265
[E3（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/src/compiler/nvidia_producer_bridge.cpp#L690
[E4（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/src/runtime/nvidia/executable.cpp#L641
[E5（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/src/runtime/nvidia/driver_api.cpp#L527
[E6（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/docs/en/dev/backend/02-nvidia-executable.md#L73
[E7（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/runs/softmax-five-task-profile-ada-sm89/summary.json
[E8（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/benchmarks/operators/profile_softmax_task_pipeline_ada_sm89.py#L71
[E9（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/runs/softmax-five-task-profile-ada-sm89/softmax-five-stage-trace.json
[E10（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/src/pypto_kernels/attention.py#L1714
[E11（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/benchmarks/paged_attention_sm120.py#L38
[F1（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/compiler.py#L1385
[F2（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/compiler.py#L194
[F3（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/driver.py#L1156
[F4（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/CMakeLists.txt#L7
[F5（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/tutorials/02-fused-softmax.py#L59
[F6（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/unittest/pytest_ut/test_triton_unified_attention.py#L37
[F7（Triton-Ascend）]: npu/torch_npu/torch_npu/_inductor/__init__.py#L125
[F8（Triton-Ascend）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HIVM/Pipelines/HIVMPipelines.cpp#L250
[F9（Triton-Ascend）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HIVM/Transforms/AutoBlockifyParallelLoop.cpp#L54
[F10（Triton-Ascend）]: npu/torch_npu/torch_npu/_inductor/codegen/npu_combined_scheduling.py#L30
[F11（Triton-Ascend）]: npu/AscendNPU-IR/bishengir/lib/Tools/bishengir-compile/regbase/PassPipeline.cpp#L239
[F12（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/utils.py#L470
[G1（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/README.md#L1
[G2（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/npu.py#L2243
[G3（AutoFuse + Inductor）]: npu/graph-autofusion/autofuse/compiler/py_module/pyautofuse.cpp#L188
[G4（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/compiler/_compiler.py#L739
[G5（AutoFuse + Inductor）]: npu/graph-autofusion/autofuse/codegen/codegen_tiling.cpp#L508
[G6（AutoFuse + Inductor）]: npu/graph-autofusion/autofuse/optimize/buffer_allocate/buf_que_allocator.cpp#L87
[G7（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/codegen/_asc_codegen.py#L610
[G8（AutoFuse + Inductor）]: npu/graph-autofusion/autofuse/compiler/python/compile_adapter.py#L577
[G9（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/tests/smoke/test_fa3_fwd_reduce_overhead.py#L24
[P1（PTOAS）]: npu/PTOAS/tools/ptoas/ptoas.cpp#L334
[P2（PTOAS）]: npu/PTOAS/lib/PTO/Transforms/PTOPlanMemory.cpp#L1
[P3（PTOAS）]: npu/PTOAS/lib/PTO/Transforms/PTOPlanMemoryModern.cpp#L1

[D14（CANNBot DSL）]: npu/cannbot-dsl_ma/examples/flash_attn_noquant/flash_attn_noquant.py#L56
[D15（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannir/cannir-opt/convert_to_ascvec_softmax.mlir#L22
[E12（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/src/pypto_kernels/attention.py#L183
[E13（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/3rdparty/nvidia/tensor-ir/include/tensor_ir/Dialect/TensorDialect.td#L9
[E14（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/3rdparty/nvidia/tensor-ir/README.md#L3
[F13（Triton-Ascend）]: npu/AscendNPU-IR/bishengir/test/Dialect/HIVM/plan-memory.mlir#L1
[G10（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/tests/smoke/inductor_npu_ext_test.py#L515
[P4（PTOAS）]: npu/PTOAS/test/samples/PyPTOIRParser/paged_attention_example_kernel_softmax_prepare.pto#L1

[A14（PyPTO2-普通版）]: npu/pypto2/framework/src/interface/configs/tile_fwk_config_schema.json#L199
[A15（PyPTO2-普通版）]: npu/pypto2/framework/src/interface/function/function.cpp#L2120
[A16（PyPTO2-普通版）]: npu/pypto2/framework/src/passes/tile_graph_pass/graph_partition/supernode_graph_builder.cpp#L1092
[A17（PyPTO2-普通版）]: npu/pypto2/python/pypto/op/__init__.py#L13
[A18（PyPTO2-普通版）]: npu/pypto2/python/pypto/op/conv.py#L191
[A19（PyPTO2-普通版）]: npu/pypto2/python/pypto/op/matmul.py#L147
[B17（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/language/__init__.py#L12
[B18（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/language/_api.py#L1396
[B19（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/language/parser/_buffer_parser.py#L10
[B20（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/language/parser/_ast_parser.py#L89
[B21（PyPTO2-Pro）]: npu/pypto2/framework/src/interface/pypto_pro/backend/backend_cce_vf_ops.cpp#L1569
[C21（PyPTO3（Simpler））]: npu/pypto/python/pypto/language/op/__init__.py#L12
[C22（PyPTO3（Simpler））]: npu/pypto/src/backend/910B/backend_910b_ops.cpp#L29
[C23（PyPTO3（Simpler））]: npu/pypto-lib/models/qwen3_14b/paged_attention_pypto.py#L260
[D16（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/channel.py#L10
[D17（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/core/frontend/compiler.py#L44
[D18（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/ops/__init__.py#L9
[D19（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/ops/reg/store.py#L155
[E15（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/.sources/pypto/src/codegen/nvidia/tensor_ir_codegen.cpp#L6968
[F14（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/__init__.py#L80
[F15（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/language/cann/extension/semantic.py#L101
[F16（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/language/cann/extension/__init__.py#L16
[G11（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/asc_ops.py#L49
[G12（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/npu.py#L3343

[A20（PyPTO2-普通版）]: npu/pypto2/examples/02_intermediate/controlflow/others/dynamic.py#L107
[B22（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/platform.py#L157
[B23（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/fa/test_flex_attention_prefill.py#L1362
[B24（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/fa/test_flex_attention_prefill.py#L1593
[B25（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/tile_based_python_programming/multi_core_partitioning_and_Tiling.md#L277
[B26（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L1746
[B27（PyPTO2-Pro）]: npu/pypto2/python/tests/ut/pypto_pro/runtime/test_shape_policy_jit.py#L31
[C24（PyPTO3（Simpler））]: npu/pypto/runtime/tests/st/a2a3/tensormap_and_ringbuffer/spmd_paged_attention_highperf/test_spmd_paged_attention_highperf.py#L295
[D20（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannbotdsl/test_aot_p1b_dyn_tail.py#L45
[D21（CANNBot DSL）]: npu/cannbot-dsl_ma/test/cannbotdsl/test_aot_p1b_dyn_tail.py#L110
[D22（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/tensor.py#L1568
[E16（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/src/pypto_kernels/_boot.py#L541
[F17（Triton-Ascend）]: npu/triton-ascend/python/triton/runtime/jit.py#L399
[F18（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/unittest/pytest_ut/test_triton_unified_attention.py#L258

[A21（PyPTO2-普通版）]: npu/pypto2/docs/zh/guide/programming_guide/tensor/debug/performance.md#L408
[A22（PyPTO2-普通版）]: npu/pypto2/docs/zh/api/tensor_api/config/pypto-set_pass_options.md#L43
[B28（PyPTO2-Pro）]: npu/pypto2/docs/zh/guide/programming_guide/pro/development/tile_based_python_programming/Reg_vector_computation.md#L1
[B29（PyPTO2-Pro）]: npu/pypto2/python/tests/st/pypto_pro/frontend/vf_api/test_layernorm_tile_group_vf.py#L71
[C25（PyPTO3（Simpler））]: npu/pypto/src/ir/transforms/outline_incore_scopes_pass.cpp#L303
[C26（PyPTO3（Simpler））]: npu/pypto/tests/ut/ir/transforms/test_outline_incore_scopes.py#L86
[C27（PyPTO3（Simpler））]: npu/pypto-lib/docs/pypto-coding/pypto-coding-style.md#L684
[C28（PyPTO3（Simpler））]: npu/pypto/python/pypto/language/op/unified_ops.py#L1135
[C29（PyPTO3（Simpler））]: npu/pypto/python/pypto/language/typing/memref.py#L46
[C30（PyPTO3（Simpler））]: npu/pypto/docs/en/dev/passes/10-convert_tensor_to_tile_ops.md#L3
[D23（CANNBot DSL）]: npu/cannbot-dsl_ma/python/cannbotdsl/core/utils/env.py#L93
[D24（CANNBot DSL）]: npu/cannbot-dsl_ma/lib/AscVec/Transforms/VfGrouping.cpp#L58
[D25（CANNBot DSL）]: npu/cannbot-dsl_ma/lib/CANNIR/Transforms/FormVfRegions.cpp#L145
[D26（CANNBot DSL）]: npu/cannbot-dsl_ma/lib/AscVec/Transforms/VfTransformPass.cpp#L123
[E17（PyPTO on GPU）]: gpu/PyPTO-LOVE-TensorIR/packages/pypto-kernels/src/pypto_kernels/sigmoid_mul.py#L22
[F19（Triton-Ascend）]: npu/torch_npu/torch_npu/_inductor/__init__.py#L379
[C31（PyPTO3（Simpler））]: npu/pypto/python/pypto/backend/pto_backend.py#L1380
[P5（PTOAS）]: npu/PTOAS/test/lit/vpto/section_sugar_multi_func.pto#L1
[P6（PTOAS）]: npu/PTOAS/tools/ptoas/VPTOHostStubEmission.cpp#L126
[P7（PTOAS）]: npu/PTOAS/tools/ptoas/ObjectEmission.cpp#L680
[P8（PTOAS）]: npu/PTOAS/test/vpto/cases/vmi_new/private-call-argument-boundary-store/launch.cpp#L28
[F20（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/include/bishengir/Dialect/HACC/IR/HACCAttrs.td#L55
[F21（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HFusion/Transforms/OpFusion.cpp#L249
[F22（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/include/bishengir/Tools/bishengir-compile/Options.td#L132
[F23（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/test/Dialect/HFusion/AutoSchedule/test-host-multiple-tiling.mlir#L1
[F24（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Conversion/HACCToLLVM/HACCToLLVM.cpp#L214
[F25（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Conversion/HACCToLLVM/GetOrCreateUtils.cpp#L27
[F26（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/hivmc/bishengir/lib/Tools/hivmc/A3/HIVMCMainA3.cpp#L936
[F27（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/hivmc/bishengir/lib/Tools/hivmc/A5/HIVMCMainA5.cpp#L861
[F28（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HFusion/Pipelines/HFusionPipelines.cpp#L289
[F29（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/include/bishengir/Dialect/HFusion/Transforms/Passes.td#L62
[F30（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/test/Conversion/HACCToLLVM/hacc-to-llvm.mlir#L1
[F31（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HFusion/Pipelines/regbase/HFusionRegbasePipelines.cpp#L502
[F32（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/compiler.py#L717
[F33（Triton-Ascend）]: npu/triton-ascend/third_party/ascend/backend/driver.py#L450
[F34（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/lib/Dialect/HIVM/Transforms/SplitMixKernel.cpp#L746
[F35（AscendNPU-IR）]: npu/AscendNPU-IR/bishengir/include/bishengir/ExecutionEngine/Passes.td#L23
[G13（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/tests/smoke/inductor_npu_ext_test.py#L523
[G14（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/npu.py#L1858
[G15（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/npu.py#L1081
[G16（AutoFuse + Inductor）]: npu/torchair/experimental/_inductor_npu_ext/python/inductor_npu_ext/npu.py#L3762
[G17（AutoFuse + Inductor）]: npu/graph-autofusion/autofuse/ascendc/api/axpy.h#L15
[B30（PyPTO2-Pro）]: npu/pypto2/python/pypto_pro/runtime/jit.py#L755
[C32（PyPTO3（Simpler））]: npu/pypto/runtime/src/a2a3/runtime/tensormap_and_ringbuffer/runtime/scheduler/scheduler_dispatch.cpp#L124
[C33（PyPTO3（Simpler））]: npu/pypto/runtime/src/a2a3/runtime/tensormap_and_ringbuffer/aicore/aicore_executor.cpp#L21
[C34（PyPTO3（Simpler））]: npu/pypto/tests/st/runtime/cross_core/test_spmd.py#L109
[C35（PyPTO3（Simpler））]: npu/pypto/runtime/src/a2a3/runtime/tensormap_and_ringbuffer/runtime/scheduler/scheduler_completion.cpp#L178
...
| D（CANNBot DSL） | **B（PyPTO2-Pro）** | 用户显式安排核内计算、缓冲/流水与launch；动态tile窗口不强制独立TilingFunc | D（CANNBot DSL）有MLIR Host/Device链和Channel协议，B（PyPTO2-Pro）有独立Pro共享IR与TileGroup体系 | 只看MLIR贯穿的编译链，F（Triton-Ascend）更近 |
